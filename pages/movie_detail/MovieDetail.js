...
Page({
  ...
  //获取影评列表
  getCommentList: function (refreshing = false) {
      if (start > total) { return }
      let _this = this;
      refreshing && this.setData({ refreshing });
      wx.showNavigationBarLoading();
      wx.request({
          url: `${app.globalData.baseURL}/v2/movie/subject/${movieId}/reviews?apikey=${app.globalData.apiKey}&start=${start}&count=${count}`,
          method: 'GET',
          header: {
              'Content-Type': 'application/x-www-form-urlencoded'
          },
   ...
   */
  dramaTap: function () {
      !this.data.unfoldDrama && this.setData({
